    ...
    <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
        ...
        <div>
          ...
          {/* Burn NFT - only visible to owner */}
          {isConnected && owner && address && (owner as string).toLowerCase() === address.toLowerCase() && (
            <button
              onClick={() => {
                if (!confirm("Are you sure you want to burn this NFT? This action is irreversible.")) return;
                writeContract(
                  {
                    address: collectionAddr,
                    abi: [
                    ...
                    args: [address, "0x000000000000000000000000000000000000dEaD", BigInt(tokenId)],
                  },
                  {
                    onSuccess: () => toast.success("NFT burned!"),
                    onError: (err) => {
                      console.error(err);
              ...
              disabled={isBuying}
              className="mt-2 w-full py-2.5 border border-red-900/50 rounded-lg text-sm text-red-400 hover:bg-red-900/20 hover:border-red-700 transition-all flex items-center justify-center gap-2"
            >
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
